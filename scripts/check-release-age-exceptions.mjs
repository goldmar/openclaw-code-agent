...
const marker = /^\s*#\s*security-exception\s+(.+)\s*$/;
const entry = /^\s*-\s*['"]?([^'"\s]+)['"]?\s*$/;
const exactPackageVersion = /^(?:@[^/@]+\/[^/@]+|[^@/]+)@(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)(?:-[0-9A-Za-z.-]+)?$/;

export function parseSecurityExceptions(source) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const fields = Object.fromEntries(
      match[1].split(/\s+/).map((part) => {
        const separator = part.indexOf("=");
        if (separator < 1) throw new Error(`Malformed security exception field on line ${index + 1}: ${part}`);
        return [part.slice(0, separator), part.slice(separator + 1)];
      }),
    );
    const packageMatch = entry.exec(lines[index + 1] ?? "");
    if (!packageMatch) throw new Error(`Security exception on line ${index + 1} must immediately precede one package entry`);
    exceptions.push({ ...fields, package: packageMatch[1], line: index + 1 });
  }
...
export function validateSecurityExceptions(source, now = new Date()) {
  ...
  for (const exception of exceptions) {
    ...
    if (!/^GHSA-[0-9a-z-]+$/.test(exception.advisory ?? "")) {
    ...
    if (![published, quarantineEnds, expires].every(Number.isFinite)) {
      throw new Error(`Security exception on line ${exception.line} requires valid published, quarantineEnds, and expires timestamps`);
    }
    if (quarantineEnds !== published + 24 * 60 * 60 * 1000) {
      throw new Error(`Security exception on line ${exception.line} must use the repository's 24-hour quarantine`);
    }
    if (expires <= quarantineEnds) {
      throw new Error(`Security exception on line ${exception.line} must expire after quarantine ends`);
    }
    if (now.getTime() >= expires) {
      throw new Error(`Security exception on line ${exception.line} expired at ${exception.expires}; remove it`);
    }
  }
  return exceptions;
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const exceptions = validateSecurityExceptions(await readFile(workspaceFile, "utf8"));
  console.log(`Validated ${exceptions.length} temporary release-age security exception(s).`);
}
