...
This release targets and validates against the OpenClaw SDK package `openclaw@2026.5.27`, while keeping the plugin peer floor at `>=2026.4.21`. No host upgrade or host config change is required for this plugin compatibility update. OpenClaw `2026.5.27` changes plugin approval action metadata, Telegram durable outbound delivery, and native callback routing; `openclaw-code-agent` keeps Start Plan, Dismiss, Approve, Revise, and Reject as plugin-owned `code-agent:<token>` callbacks and accepts the current Telegram callback context shape.
