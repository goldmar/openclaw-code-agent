...
The current package targets and requires OpenClaw `2026.8.1`. The manifest still declares `contracts.tools`, the plugin still imports only `openclaw/plugin-sdk/plugin-entry`, and its session store, wake routing, callbacks, worktree flows, and Codex/Claude model restrictions remain plugin-owned. OpenClaw's host-side migration of `codex/*` and `openai-codex/*` model routes to `openai/*` does not rewrite Code Agent's unprefixed Codex harness models (`gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`) or bypass `harnesses.codex.allowedModels`. Existing sessions, permissions, Telegram/topic routes, automation delivery context, callback ownership, and tool allowlists remain under the same plugin contracts; installing this package performs no host-side configuration migration.
