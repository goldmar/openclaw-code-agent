...
The current package targets and validates against the OpenClaw SDK package `openclaw@2026.6.9`, while keeping the plugin peer floor at `>=2026.4.21`. No host upgrade or host config change is required for this plugin compatibility update. OpenClaw `2026.6.9` changes installed channel plugin discovery, provider package boundaries, plugin write ownership checks, declared tool allowlist recognition, cron delivery defaults, yielded media handling, agent completion recovery, and channel reply reliability. Current main also includes the recent callback and follow-up fixes for native Telegram/Discord callback data, delayed/raw callback routing, retryable plan/question tokens, serialized plan decisions, and PR update completion summary dedupe. `openclaw-code-agent` does not need new SDK imports for the OpenClaw changes: the manifest already declares `contracts.tools`, the plugin still imports only `openclaw/plugin-sdk/plugin-entry`, and its own session store, wake routing, callbacks, worktree flows, and harness model restrictions remain plugin-owned.
