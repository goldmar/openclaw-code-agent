...
- **Hardened.** A release gate runs ClawHub's static scan over the exact packed files and fails on any finding. The security review ships in the package. Tests can no longer write to your real `~/.openclaw`, and a class of Gateway crashes from orphaned Codex requests is fixed.
