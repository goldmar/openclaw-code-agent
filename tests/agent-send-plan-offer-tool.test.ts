...
describe("agent_send_plan_offer tool", () => {
  ...
  it("routes an interactive plan offer through SessionManager", async () => {
    ...
    const result = await tool.execute("tool-id", {
      offer_id: "plugin-readiness-v2026.5.12-beta.1",
      ...
      plan_name: "plugin-readiness-v2026.5.12-beta.1",
    });

    assert.equal(calls.length, 1);
    assert.equal(calls[0]?.offerId, "plugin-readiness-v2026.5.12-beta.1");
