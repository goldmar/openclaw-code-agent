...
describe("plugin entry source", () => {
  ...
  it("keeps package and plugin manifest versions in sync", () => {
    ...
    assert.equal(openclawVersion, "2026.5.7");
    assert.equal(pluginSdkVersion, "2026.5.7");
  ...
  it("declares the v2026.4.21 compatibility floor and v2026.5.7 SDK readiness target in package metadata", () => {
    const packageJson = JSON.parse(readFileSync(join(rootDir, "package.json"), "utf8")) as {
    ...
    assert.equal(packageJson.openclaw?.build?.openclawVersion, "2026.5.7");
    assert.equal(packageJson.openclaw?.build?.pluginSdkVersion, "2026.5.7");
    assert.equal(packageJson.peerDependencies?.openclaw, ">=2026.4.21");
    assert.equal(packageJson.devDependencies?.openclaw, "2026.5.7");
  ...
  it("documents 2026.5.7 plugin allowlist and apply_patch policy boundaries", () => {
    const reference = readFileSync(join(rootDir, "docs", "REFERENCE.md"), "utf8");

    assert.match(reference, /OpenClaw 2026\.5\.7 SDK Readiness/);
    assert.match(reference, /package build metadata targets OpenClaw `2026\.5\.7` for both host and SDK readiness/);
