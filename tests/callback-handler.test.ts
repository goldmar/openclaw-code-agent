...
describe("createCallbackHandler()", () => {
  ...
  it("launches a plan-only session from plan-offer actions", async () => {
    ...
    setSessionManager({
      getActionToken: () => ({
        sessionId: "plugin-readiness-v2026.5.5",
        ...
        launchName: "plugin-readiness-v2026.5.5",
      ...
      launchPlanOffer: (args: Record<string, unknown>) => {
        launches.push(args);
        return { id: "sess-plan", name: "plugin-readiness-v2026.5.5" };
      },
    } as any);
    ...
    assert.equal(launches[0]?.name, "plugin-readiness-v2026.5.5");
    ...
    assert.match(state.replies[0], /Planning session started: plugin-readiness-v2026\.5\.5 \[sess-plan\]/);
  });
});
