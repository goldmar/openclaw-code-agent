...
describe("createCallbackHandler()", () => {
  ...
  it("launches a plan-only session from plan-offer actions", async () => {
    ...
    setSessionManager({
      getActionToken: () => ({
        sessionId: "plugin-readiness-v2026.5.12-beta.1",
        ...
        launchName: "plugin-readiness-v2026.5.12-beta.1",
      ...
      launchPlanOffer: (args: Record<string, unknown>) => {
        launches.push(args);
        return { id: "sess-plan", name: "plugin-readiness-v2026.5.12-beta.1" };
      },
    } as any);
    ...
    assert.equal(launches[0]?.name, "plugin-readiness-v2026.5.12-beta.1");
    ...
    assert.match(state.replies[0], /Planning session started: plugin-readiness-v2026\.5\.12-beta\.1 \[sess-plan\]/);
  });
});
