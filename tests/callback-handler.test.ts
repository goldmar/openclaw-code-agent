...
describe("createCallbackHandler()", () => {
  ...
  it("continues Telegram plan approval when structured markup cleanup reports no change", async () => {
    const events: string[] = [];
    const session = createStubSession({
      pendingPlanApproval: true,
      approvalState: "pending",
  ...
  it("saves repo policy, clears only button markup, and continues the stored launch", async () => {
    ...
    setSessionManager({
      ...
      launchAfterRepoPolicyChoice: (args: Record<string, unknown>) => {
        ...
        assert.equal(args.rewindTurns, 2);
