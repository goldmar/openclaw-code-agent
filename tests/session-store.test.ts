...
describe("SessionStore path resolution", () => {
  ...
  it("archives legacy array stores to a suffixed path when the timestamp target exists", (t) => {
    ...
    mkdirSync(`${indexPath}.legacy-${now}.json`);
  ...
  it("archives corrupt JSON stores and writes a clean current-schema index", () => {
    const dir = mkdtempSync(join(tmpdir(), "openclaw-store-corrupt-"));
    const indexPath = join(dir, "sessions.json");
    const corruptPayload = "{not-json";
    writeFileSync(indexPath, corruptPayload, "utf-8");
  ...
  it("archives invalid current-schema stores to a suffixed path when the timestamp target exists", (t) => {
    ...
    const originalPayload = JSON.stringify({
      ...
      sessions: [
        {
          sessionId: "valid-route",
          harnessSessionId: "h-valid-route",
          backendRef: { kind: "claude-code", conversationId: "thread-valid-route" },
          name: "valid-route",
          prompt: "p",
          workdir: "/tmp",
          status: "completed",
