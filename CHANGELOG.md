...
- **Orchestrator contract (wakes).** Wake texts were rewritten to be short: the session is named `[name]`, the origin route appears once as `originRoute: {…}`, and approval-state lines appear only for an anomaly (`implemented_without_required_approval`). Prompts or parsers that matched the 4.x wording (`[DELEGATED PLAN APPROVAL]`, `[DELEGATED WORKTREE DECISION]`, `Completion diagnostics:`, `Session origin route (authoritative …)`, `Requested permission mode:`) must be updated. Context the orchestrator needs only if the user answers in chat (a question or plan prompt that reached the user, a worktree prompt with buttons, a Revise press) is queued for its next turn instead of starting a turn; the "repo policy buttons delivered" wake and the plan-timeout wake for a prompt the user already has are no longer sent. A wake with an origin route tells the orchestrator to reach the user with the message tool to `originRoute` and then answer `NO_REPLY` (a plain wake reply stays internal for session keys without the chat's channel, such as `dmScope: "per-peer"` keys, and is never delivered under the Codex runtime's message-tool reply mode); `originRoute` no longer includes the session key. `agent_launch` waits up to 4 seconds (less when the agent starts working) and reports a session that already failed or finished in its result, so the launching turn tells the user; that outcome's wake is then skipped. Otherwise the result tells the launching turn to end after announcing the launch, without waiting (`sessions_yield`, polling) or describing the session's progress later in the turn, which could go stale if the session finishes meanwhile. A session that fails or completes within a minute of launch holds its wake for 15 seconds and skips it when the launching orchestrator session already read the outcome with `agent_output` (a read from any other session does not count).
...
- `agent_goal` `max_cost_usd` / `/agent_goal --max-cost-usd`: stop a goal loop before the next iteration once its sessions cost that much. Sessions that bill nothing per token (Codex with a ChatGPT login) count at the API-price estimate of their tokens.
...
- Legacy `agent_worktree_cleanup(dry_run=true)` calls now fail with the replacement preview syntax instead of silently using the 5.0 cleanup default. A stale worktree reminder reserves its slot when the host accepts a durable send, preventing a duplicate reminder after an uncertain platform outcome.
- The plan decision brief keeps simple one-line fenced code with its step, and shows multiline, quoted, or long fenced code verbatim so executable text keeps its meaning. A plan with a section heading that maps to no brief field (`## Current file`, `## Commit`) is shown as the complete paginated plan (headed `Plan`) instead of a mislabeled brief.
- A Revise button press or a forwarded bare `revise` followed by the change produced `Plan v3` right after `Plan v1`: the change request cleared the plan context, so the revised plan counted as a new plan. It is now `Plan v2`. A bare `revise` forwarded by the orchestrator gets its next step in the tool result instead of a next-turn note that arrived after the revised plan, and the tool, the skill and the plan notes say to forward each user message whole in one call (a model had split "Revise the plan: …" into `revise` plus the change).
- The worktree prompt's next-turn note to the orchestrator names only the buttons the user got (no `Open PR` when the repository has no PR provider).
