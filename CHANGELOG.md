...
- Codex runs sandboxed by default: `harnesses.codex.permissionProfile` defaults to `:workspace`, `approvalPolicy` to `on-request`, and `approvalsReviewer` to `auto_review` (previously `:danger-full-access`, `never`, `user`). Codex can write inside the workspace; network access (for example `git push` to a real remote or package installs) and writes outside the workspace are escalations that Codex's reviewer subagent approves or denies without a chat prompt. Live testing showed `git add`, `git commit`, and a local push each approved in 3–5 s; network access and pushes to a real remote under `:workspace` have not been exercised yet. To restore the 4.x behavior, set `harnesses.codex.permissionProfile: ":danger-full-access"` and `approvalPolicy: "never"`.
...
- Codex approvals: `harnesses.codex.permissionProfile` (`:workspace` default, `:read-only`, `:danger-full-access`), `harnesses.codex.approvalPolicy` (`on-request` default, `untrusted`, `never`), and `harnesses.codex.approvalsReviewer` (`auto_review` default, `user`). Command, file-change, and permission requests are shown as approval buttons with typed decisions (including "Always allow `<prefix>`"); unrecognized text replies decline and are steered into the turn as feedback.
...
- `.openclaw/worktree-setup.sh` always runs for OCA worktrees, unlike OpenClaw core, which runs it only for admin-scope callers of `worktrees.create`. An OCA worktree exists only for a coding session the orchestrator launched in an operator-chosen repository, so running that repository's own setup script is part of trusting it. The script runs unsandboxed with the Gateway's privileges (more than a default `:workspace` Codex session).
