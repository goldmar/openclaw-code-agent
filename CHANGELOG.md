...
- `planApproval: "ask"`: `agent_respond(approve=true)` from the orchestrator is refused; only the user's Approve button or the user's own words forwarded with `userInitiated=true` approve the plan.
...
- Codex plan turns run with the `:read-only` permission profile; the first turn after approval restores the configured profile. Codex's plan collaboration mode alone only instructs the model, and the default thread sandbox is `:danger-full-access`.
...
- Codex approvals: `harnesses.codex.permissionProfile` (`:danger-full-access`, `:workspace`, `:read-only`), `harnesses.codex.approvalPolicy` (`never`, `on-request`, `untrusted`), and `harnesses.codex.approvalsReviewer` (`user`, `auto_review`). When unset they follow the host `tools.exec.mode` like OpenClaw's bundled Codex plugin: unset or `full` keeps full access with no Codex prompts (the 4.x behavior), `auto` uses `:workspace` + `on-request` + `auto_review`, `ask` uses the same sandbox with approvals in chat, and `deny` / `allowlist` refuse Codex launches unless `permissionProfile` is set. Explicit values always win, unlike the bundled plugin, where `tools.exec.mode: "auto"` overrides them. The mode is read from the live config at each Codex launch. Command, file-change, and permission requests are shown as approval buttons with typed decisions (including "Always allow `<prefix>`"); unrecognized text replies decline and are steered into the turn as feedback.
...
- The worktree decision summary and `agent_pr`'s PR metadata waited on `runtime.llm` without a time limit, so a hung host completion held the decision prompt or the PR back indefinitely. They are now bounded (20 s and 45 s), aborted through the request signal, and fall back to the deterministic summary; the question summary uses the same helper.
...
- `agent_request_plan_approval` refused to escalate `approve`-mode plan reviews ("already uses direct user plan approval"), although the approve-mode wake tells the orchestrator to send risky plans to the user with it. Only `ask` mode, where the user already has the prompt, is refused now.
...
- A Codex `agent_session_action` (review/compact) queued while a turn was running could crash the Gateway: after the session ended, the queued request's write failed but its pending entry stayed registered, and closing the client rejected that orphaned promise unhandled (`codex app server stdio closed`). The RPC client now registers a request only after a successful write, the Codex harness never starts work after the session ended (actions reject with a clear error), and `agent_session_action` refuses sessions that are not running instead of replying "queued". Detached promises in the Codex, OpenCode, and Claude Code harnesses now have handlers.
...
- LLM summaries (worktree decisions, question context, PR metadata) failed with `Async work scope is closed` because they ran in the async context of a finished tool call; they now run detached from it.
...
- OpenCode: the first request after the shared `opencode serve` restarted could fail with "fetch failed" (ECONNRESET). `--port 0` usually binds the same port (4096) again, and Node's fetch reused a kept-alive socket to the server that had just stopped. OCA now opens a fresh connection for every request to the local server (`connection: close`) and repeats a read (`GET`) once when its connection is reset.
...
- `.openclaw/worktree-setup.sh` always runs for OCA worktrees, unlike OpenClaw core, which runs it only for admin-scope callers of `worktrees.create`. An OCA worktree exists only for a coding session the orchestrator launched in an operator-chosen repository, so running that repository's own setup script is part of trusting it. The script runs unsandboxed with the Gateway's privileges (which can be more than a `:workspace` Codex session has).
