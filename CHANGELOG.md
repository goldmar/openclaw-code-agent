...
- `agent_kill(session, forget=true)` and `/agent_kill --forget <session>` delete a finished session's stored record, action tokens, schedules, and output file. They refuse running or suspended sessions, sessions whose worktree is not settled (a pending decision, a merge or PR in progress, an open PR, or a worktree directory still on disk), sessions with a PR that `gh` does not confirm as merged or closed, sessions still delivering their final notification, and sessions that a running goal loop owns.
