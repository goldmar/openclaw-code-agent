...
- Dependency updates: `@anthropic-ai/sdk` 0.128.0, `@modelcontextprotocol/sdk` 1.30.1, `hono` 4.13.9, `zod` 4.6.5, and, for development, `esbuild` 0.28.2 and `tsx` 4.23.15. The pinned runtime security floors live in one place: `package.json` `dependencies` holds the versions, `scripts/lib/runtime-dependency-pins.mjs` names the packages, and the shrinkwrap check, the packed-consumer check, and the matching `pnpm-workspace.yaml` overrides (now checked to equal `package.json`) read from there.
