...
- The plan decision brief keeps a fenced code block with the step that introduces it (as inline code) instead of splitting its lines into brief fields (an `Objective / scope` field showing only the opening fence). A plan with a section heading that maps to no brief field (`## Current file`, `## Commit`) is shown as the plan itself (headed `Plan`, shortened past 2,400 characters) instead of a mislabeled brief.
