    ...
    if (resolvedMode === "delegate") {
      return {
        ...
        wakeMessage: [
          ...
          ``,
          `If approving: call agent_respond(session='${session.id}', message='Approved because <brief reason>. Go ahead.', approve=true).`,
          `That brief reason should say why approval was safe: for example scope match, low risk, or no meaningful ambiguity.`,
