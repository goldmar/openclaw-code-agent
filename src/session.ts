    ...
    this.harnessEvents = new SessionHarnessEventApplier({
      ...
      assignBackendRef: (ref) => {
        ...
        if (ref.worktreePath) {
          ...
          const nativeWorktreePath = ref.worktreePath;
          if (this.worktreeBranch === undefined) {
            void getBranchName(nativeWorktreePath).then((branch) => {
  ...
  async sendMessage(text: string): Promise<void> {
    ...
    if (this.pendingModeSwitch) {
      ...
      if (await this.resolveNativePlanDecision({ kind: "approve", permissionMode: newMode })) {
        ...
        if (isBareApprovalMessage(text)) return;
      ...
      if (await this.resolveNativePlanDecision({ kind: "revise", feedback: text })) return;
