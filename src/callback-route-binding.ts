 ...
 * delivered to a chat) and a callback without conversation details cannot be
 * compared and pass; the host's sender check still applies to them.
 */
export function callbackMatchesTokenRoute(conversation: CallbackConversation, route: SessionRoute | undefined): boolean {
  ...
  if (provider === "discord") {
    ...
    if (!expected || actual.length === 0) return true;
