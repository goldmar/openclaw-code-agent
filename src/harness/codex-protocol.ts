...
function extractTopLevelOptions(value: unknown): PendingInputOption[] {
  const record = asRecord(value);
  if (!record) return [];
...
function extractQuestionList(value: unknown): PendingInputQuestion[] {
  const record = asRecord(value);
...
function formatPendingInputQuestions(questions: PendingInputQuestion[]): string | undefined {
