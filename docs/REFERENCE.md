...
- The server runs under a small watchdog (a second process of the Gateway's Node executable) that leads its own process group: closing the server stops it and every tool process it started, and when the Gateway exits for any reason, including SIGKILL, the watchdog sees its stdin pipe close and stops the group.
