...
- The session's reasoning effort is sent as the prompt's `variant` when the model defines it. On the first turn OCA reads the model from `GET /config/providers`: an effort the model has no variant for is not sent (and logged), and the session reports the model, the applied effort and the model's context window.
...
- Subagents (the `task` tool) run in child sessions. OCA follows their `session.created` events, gives each subagent session the parent's permission overlay (so `default` asks and `plan` denies inside subagents too), and shows their permission prompts and questions like the session's own ("An OpenCode subagent requests …"). A subagent going idle does not end the parent's turn.
...
- The server leads its own process group, so closing it stops every tool process it started as well. A small watchdog (a second process of the Gateway's Node executable, which starts nothing itself) holds a pipe from the Gateway: when the Gateway exits for any reason, including SIGKILL, the pipe closes and the watchdog stops the server's process group.
