...
### OpenClaw 2026.6.6 SDK Readiness

The current `openclaw-code-agent` package is validated against the OpenClaw SDK package `openclaw@2026.6.6`. The plugin keeps the peer floor at `>=2026.4.21` for compatible existing installs, while package build metadata targets OpenClaw `2026.6.6` for both host and SDK readiness.

No host upgrade, `openclaw doctor --fix`, Gateway restart, or host config change is required for this plugin compatibility update. No plugin compatibility source update was needed for `2026.6.6`: the release adds `WakeParams.agentId`, Telegram delivery and callback hardening, cron update model-clearing support, runtime tool allowlist changes, installed-plugin trusted policy contracts, and material bundled Codex/ACPX package updates, but this plugin does not depend on those new imports. Runtime code still imports only `openclaw/plugin-sdk/plugin-entry` from the OpenClaw SDK, `openclaw.plugin.json` already declares tools through `contracts.tools`, and code-agent session storage remains plugin-owned.

Configuration guidance for `2026.6.6`:
...
- Cron run logs, delivery validation, payload fallback handling, and model override forwarding are harder in `2026.6.6`; API `cron.update` payload patches can also set `model: null` to clear a stored job model override. Code-agent wakeups from scheduled or release-monitor style turns should keep using stored delivery context so no-change, completion, Start Plan, merge, and PR summaries return to the originating route/topic.
