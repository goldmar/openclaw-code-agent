...
### OpenClaw 2026.8.1 SDK Readiness
...
- Managed external-plugin npm installs enforce `openclaw.compat.pluginApi` when selecting compatible package versions. This package aligns that range, its Gateway minimum, and its peer dependency with OpenClaw `2026.8.1`. Keep `openclaw.extensions` pointing at the built `dist/index.js` artifact.
