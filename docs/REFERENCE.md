...
- Telegram topic routes remain plain code-agent route metadata. OpenClaw `2026.7.1` adds durable turn adoption, safer final-send recovery, and further account-scoped topic handling; for cron or release-monitor wakeups, still use fully routable channel strings and preserve topic/thread ids such as `telegram|<chat-id>` with thread `<topic-id>`. The plugin's Start Plan, approval, completion, merge, and PR callbacks continue to route through stored delivery context and action tokens.
...
- OCA self-update reports success but stays on the old version: OpenClaw `2026.7.1` can treat `openclaw plugins update openclaw-code-agent@<version>` as an install-record ID, print `No install record for "openclaw-code-agent@<version>".`, and exit `0`. Current OCA avoids that selector: npm installs use `openclaw plugins install <recorded-npm-package>@<approved-version> --force`, ClawHub installs use `openclaw plugins install clawhub:<recorded-package>@<approved-version> --force`, and success requires a matching `plugins inspect --runtime --json` result plus matching managed source, package, and version metadata.
