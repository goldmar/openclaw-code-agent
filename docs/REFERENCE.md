...
- If `ExitPlanMode` carries neither `plan` nor `planFilePath`, the pending plan is read from the newest Markdown file written to a Claude plans directory since the session launched, so `agent_output` and the approval prompt still show it.
- A forked session reports only its own cost: the parent's usage at fork time is subtracted from the SDK totals.
