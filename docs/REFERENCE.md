...
- Subagents (the `task` tool) run in child sessions. OCA follows their `session.created` events, gives each subagent session the parent's permission overlay as soon as it is created (so `default` asks and `plan` denies inside subagents too; a subagent whose overlay cannot be applied is aborted), and shows their permission prompts and questions like the session's own ("An OpenCode subagent requests …"). A subagent going idle does not end the parent's turn.
