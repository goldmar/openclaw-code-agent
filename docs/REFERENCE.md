...
- The server leads its own process group and carries a random marker in its environment (`OPENCLAW_CODE_AGENT_LIFELINE`) that every process it starts inherits. Closing it signals the group and, on Linux, every process with the marker (OpenCode's shell tool starts commands in process groups of their own, so the group alone would miss them). A small watchdog (a second process of the Gateway's Node executable, which starts nothing itself) holds a pipe from the Gateway: when the Gateway exits for any reason, including SIGKILL, or when the server exits on its own, it stops the group and the marked processes. On macOS only the group is signalled, so a tool that moved to its own process group can outlive a server that exits.
