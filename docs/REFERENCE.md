...
- Completion wakes deliver the canonical plugin status first and request at most one orchestrator follow-up when `completionWakeSummaryRequired=true`; an empty response is not delivery proof. The host delivers the orchestrator's reply to a wake itself only when the origin session key names the chat's channel with a peer shape (`agent:<id>:telegram:group:…`, `agent:<id>:slack:direct:…`) or is the main session (`agent:<id>:main`). For any other key (`dmScope: "per-peer"` keys such as `agent:<id>:direct:<peer>`, cron, sub-agent, ACP or custom keys) the reply stays internal, so the wake tells the orchestrator to send its message with the message tool to `originRoute` and then answer `NO_REPLY`. PR update completion summaries are deduplicated by material outcome, so later updates with new commits still produce a fresh summary.
