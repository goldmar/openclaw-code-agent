...
Configuration guidance for `2026.5.22`:
...
- Telegram topic routes remain plain code-agent route metadata. For cron or release-monitor wakeups, use fully routable channel strings and preserve topic/thread ids such as `telegram|-1003863755361` with thread `13832`; the plugin's Start Plan, approval, completion, merge, and PR callbacks continue to route through stored delivery context and action tokens.
...
- OpenClaw Codex app-server changes include compaction timeout recovery, post-tool idle completion handling, web-search transcript mirroring, slow dynamic `message` sends, replay-safe client-close handling, and native subagent task mirroring; this plugin keeps its Codex harness isolated from bundled Codex provider/runtime config.
