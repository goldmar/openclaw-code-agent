...
| Goal verifier | `src/goal-controller.ts` | `bash` runs each command from `agent_goal(action="launch", verifier_commands=...)` in the goal's workdir. The orchestrator model supplies these commands, so they are not operator input: OCA shows the exact list to the user, who confirms it once when the goal launches, and nothing runs before that. Verifiers run with a minimal environment instead of the Gateway's. Do not expose goal launches to untrusted users. |
...
Plan mode is not a sandbox for Codex. OCA's plan mode selects Codex's `plan` collaboration mode, which tells the model to plan instead of editing, but the thread keeps the permission profile above. A live check with Codex CLI 0.156.1 under the default `:danger-full-access` + `never` posture confirmed it: a plan turn asked to run `python3 -c 'import calc'` wrote `__pycache__` before the plan was approved (the model refuses explicit write requests on its own, but nothing enforces it). Forcing `:read-only` during plan review was evaluated for 5.0.0 and left out: under `tools.exec.mode: "auto"` and `"ask"` the plan turn still wrote through sandbox escalations. Set `permissionProfile` to `:workspace` or `:read-only` when plan-time writes matter.
...
OCA makes one outbound request of its own: the npm update check, a bounded HTTPS `GET https://registry.npmjs.org/openclaw-code-agent/latest` with a 10 s timeout, sent only for npm installs and only while `autoUpdate` is on. It lives in its own bundle chunk (`dist/chunks/npm-release-client-*.js`), which reads no environment variables and sends no local data. ClawHub installs check through `openclaw plugins search` instead. Apart from that, OCA talks only to local processes: stdio JSON-RPC to Codex, and HTTP to the shared `opencode serve` on 127.0.0.1. That server is unauthenticated unless `OPENCODE_SERVER_PASSWORD` is set (OCA then sends Basic auth): while it runs, any local user or process that can reach 127.0.0.1 can drive it, including prompting sessions that run with OpenCode's tool permissions. On a shared host, set `OPENCODE_SERVER_PASSWORD` in the Gateway environment. Coding-agent backends make their own model-provider requests with their own credentials; OCA does not read or forward those credentials.
