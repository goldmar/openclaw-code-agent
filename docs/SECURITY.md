...
| OpenCode server | `src/harness/opencode.ts`, `src/harness/process-lifeline.ts` | One shared `opencode serve --hostname 127.0.0.1 --port 0 --print-logs` (override the binary with `OPENCLAW_OPENCODE_COMMAND`), started through a watchdog: `node -e <fixed watchdog source> <opencode> serve ...` with the Gateway's Node executable. The watchdog leads its own process group and stops the server and its tool processes when the Gateway's stdin pipe closes (also on SIGKILL) or when OCA closes the server. Started lazily for the first OpenCode session, addressed through the URL it prints, and shut down about 30 seconds after the last OpenCode session ends. Binds to localhost only and requires a random per-spawn password; see [Network](#network). |
