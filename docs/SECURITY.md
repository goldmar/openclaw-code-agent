...
| OpenCode server | `src/harness/opencode.ts`, `src/harness/process-lifeline.ts` | One shared `opencode serve --hostname 127.0.0.1 --port 0 --print-logs` (override the binary with `OPENCLAW_OPENCODE_COMMAND`), in its own process group, with a random marker in its environment that its tool processes inherit. A watchdog, `node -e <fixed source> <server pid> <marker>` with the Gateway's Node executable, holds a pipe from the Gateway; when the pipe closes (the Gateway exited, even on SIGKILL) or the server has exited, it signals the server's process group and, on Linux, the processes whose `/proc/<pid>/environ` carries the marker and the live server's descendants. It starts nothing itself. Started lazily for the first OpenCode session, addressed through the URL it prints, and shut down about 30 seconds after the last OpenCode session ends. Binds to localhost only and requires a random per-spawn password; see [Network](#network). |
