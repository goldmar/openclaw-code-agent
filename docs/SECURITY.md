...
Codex's `plan` collaboration mode on its own only tells the model to plan instead of editing; the thread keeps the permission profile above (a live check with Codex CLI 0.156.1 showed a plan turn under the default `:danger-full-access` + `never` posture running with a full-access sandbox, relying on the model to refrain from writing). OCA therefore runs every plan turn with the `:read-only` permission profile and restores the configured profile on the first turn after approval, so a plan turn cannot write files before the plan is approved.
