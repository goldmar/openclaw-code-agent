...
Coding-agent processes (the Claude Code executable, the Codex App Server, and the OpenCode server) and OCA's `git` / `gh` calls inherit the Gateway's environment: the agents read their own provider credentials and configuration from it. Anything in the Gateway environment is therefore visible to the coding agents and to the commands they run. Keep credentials the agents should not see out of the Gateway environment. The worktree setup script and goal verifiers are the exceptions above: they get a minimal environment.
...
Plan mode is not a sandbox for Codex. OCA's plan mode selects Codex's `plan` collaboration mode, which tells the model to plan instead of editing, but the thread keeps the permission profile above. With the default full-access posture, a Codex plan turn can technically write files and run commands before the plan is approved. Set `permissionProfile` to `:workspace` or `:read-only` when that matters.
