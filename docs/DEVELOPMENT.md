...
Pinned runtime security floors have one source of truth: the exact versions in `package.json` `dependencies`. `scripts/lib/runtime-dependency-pins.mjs` lists which packages are pinned floors; `scripts/check-npm-shrinkwrap.mjs` (part of `pnpm check-static-guardrails`) requires each to be an exact direct dependency that the shrinkwrap resolves, and requires any `pnpm-workspace.yaml` override of the same package to read `<name>@<<version>: <version>` with the `package.json` version. `pnpm verify:npm-consumer` reads the same versions. To raise a floor, change `package.json` and the matching override, then regenerate both lock files.
