...
Release-prep docs should also cover behavior that changed since the previous tag. For `4.3.0`, that includes Codex `harnesses.codex.fastMode` mapping to the current App Server `service_tier: "fast"` contract, Codex `reasoningEffort` staying on the current `reasoningEffort` field, and the orchestrator-owned routed follow-up summary after merge/PR worktree outcomes.
