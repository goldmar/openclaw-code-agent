...
- The slot records a build identity (package version plus a digest of the entry module) and the build of the most recent registration. Only that build may run the runtime: it takes over from an older build's runtime by stopping it first (the same persist-and-stop as a service stop) and then loading the store, and an older build refuses to start, even when no runtime is running, instead of creating a second writer. A newer registration of the same build with different plugin settings rebuilds the runtime the same way, so services always use the current settings.
