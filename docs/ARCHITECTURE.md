...
Launch notifications and assistant kickoff replies have separate owners: the plugin emits the launch lifecycle notification, while the foreground assistant owns any explicit kickoff summary in the chat. Completion-summary dedupe does not hide either message; it applies to terminal/worktree outcome follow-ups after the canonical lifecycle status has been delivered.
