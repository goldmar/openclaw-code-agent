...
Worktree terminal outcomes use a single-summary UX contract. A wake with `completionWakeSummaryRequired=true` makes the orchestrator read `agent_output(..., full=true)` and send one factual result; the plugin's prepared status is a direct routed fallback only when that wake fails. The wake includes authoritative outcome facts and the origin route block, including the PR URL for PR outcomes. If the active row no longer has origin metadata, persisted `route` remains the source of truth so delivery preserves provider, target, and thread/topic. The pending repair flag is cleared only when the routed wake succeeds with a non-empty final response that is not `NO_REPLY`; recovered internal failures and intermediate lifecycle telemetry do not create user notifications.
